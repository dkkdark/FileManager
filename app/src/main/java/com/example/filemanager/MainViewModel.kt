...
class MainViewModel @Inject constructor(
    ...
    // There are too many file, so let's limit some unnecessary directories
    ...
    private suspend fun insertHashes(directory: File){
        withContext(Dispatchers.IO) {
            directory.walkTopDown()
                .onEnter { file -> excludedDirs.any { !file.absolutePath.contains(it) } }
                .filter { it.isFile }
                .forEach { file ->
